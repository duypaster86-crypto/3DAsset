# Slice/Task slice-25 — Wishlist khách hàng
...
Wishlist khách hàng hoạt động xuyên suốt UI → API → Service → Database/đối tác liên quan, tạo ra một luồng người dùng có thể tự bấm và kiểm chứng.

## Task

| # | Task | Tầng (nếu dùng Lát cắt ngang) | Ghi chú |
| --- | --- | --- | --- |
| 1 | Tạo thêm/xóa sản phẩm yêu thích với ràng buộc không trùng. | — | Bao gồm code, migration/contract liên quan và test nhỏ nhất phù hợp. |
| 2 | Tạo trang wishlist có tìm kiếm và chuyển sản phẩm sang giỏ. | — | Bao gồm code, migration/contract liên quan và test nhỏ nhất phù hợp. |
| 3 | Hiển thị trạng thái đã sở hữu/ẩn/ngừng bán và test phân quyền dữ liệu. | — | Bao gồm code, migration/contract liên quan và test nhỏ nhất phù hợp. |

## Stub cho phép

- Nội dung demo, ảnh mẫu và dữ liệu seed không nhạy cảm có thể dùng tạm nếu được đánh dấu `TODO(slice-25)`.
...
Khách lưu sản phẩm, đăng nhập lại vẫn thấy và chuyển được sang giỏ.
