# Slice/Task slice-0 — Walking skeleton
...
Walking skeleton hoạt động xuyên suốt UI → API → Service → Database/đối tác liên quan, tạo ra một luồng người dùng có thể tự bấm và kiểm chứng.

## Task

| # | Task | Tầng (nếu dùng Lát cắt ngang) | Ghi chú |
| --- | --- | --- | --- |
| 1 | Dựng monorepo FE/BE, cấu hình local và endpoint health-check thật. | — | Bao gồm code, migration/contract liên quan và test nhỏ nhất phù hợp. |
| 2 | Nối PostgreSQL bằng migration đầu tiên và trả trạng thái kết nối qua API. | — | Bao gồm code, migration/contract liên quan và test nhỏ nhất phù hợp. |
| 3 | Tạo trang web gọi API, hiển thị trạng thái hệ thống và thêm smoke test CI. | — | Bao gồm code, migration/contract liên quan và test nhỏ nhất phù hợp. |

## Stub cho phép

- Nội dung demo, ảnh mẫu và dữ liệu seed không nhạy cảm có thể dùng tạm nếu được đánh dấu `TODO(slice-0)`.
...
Mở trang hệ thống và thấy frontend → backend → PostgreSQL đều hoạt động thật.
